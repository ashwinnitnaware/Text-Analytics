...
if (!Gate.isInitialised()){
			...
			Gate
					...
					.registerDirectories(
							new File(
									"F:/Sentimental Analysis/Text Analytics/gate-7.0-build4195-ALL/gate-7.0-build4195-ALL/plugins/ANNIE")
									.toURI().toURL());
	 ...
	 * @return
	 */
	public static SerialAnalyserController createResource(String[] arrModel,
			FeatureMap[] params) {
		...
		while (iter.hasNext()) {
			...
			if (originalContent != null) {
				...
				for (int i = 0; i < sortedAnnotations_token.size(); i++) {
					...
					if (sentenceSplitSet.contains(currAnnot.getStartNode()
							.getOffset())
							&& currAnnot.getStartNode().getOffset() != 0) {
						Long startOffset = currAnnot.getStartNode().getOffset();
						// Long endOffset = currAnnot.getEndNode().getOffset();
						docData.getSentences().add(
								docStr.substring(prevOffset, startOffset
										.intValue()));
						docData.getSentSplitOffsets().add(startOffset);
						docData.getPOSTagSent().add(
								posTagSent.toString().trim());
					...
					if (annottype.equalsIgnoreCase("Token")) {
						...
						posTagSent.append(word).append("/").append(posTag)
								.append(" ");
						mapAnnotations.put(
								currAnnot.getStartNode().getOffset(), word);
						mapIdToOffsetOriginal.put(i, currAnnot.getStartNode()
								.getOffset());
				...
				docData.getSentences().add(
						docStr.substring(prevOffset, docStr.length()));
				...
				try {
					...
					for (int i = 0; i < sortedAnnotations_token.size(); i++) {
						...
						tokens.append(temp);
						if (!(temp.equals(".") | temp.equals(",") | temp
								.equals(";")))
							tokens.append(" ");
					}
				...
				while (iter_stem.hasNext()) {
					...
					AnnotationSet defaultAnnotSet_stem = doc_stem
							.getAnnotations();
					AnnotationSet annSetToken_stemmed = defaultAnnotSet_stem
							.get("Token");
					it = annSetToken_stemmed.iterator();
					SortedAnnotationList sortedAnnotations_stem_token = new SortedAnnotationList();
					while (it.hasNext()) {
						currAnnot = (Annotation) it.next();
						sortedAnnotations_stem_token
								.addSortedExclusive(currAnnot);

					} // while

					for (int j = 0; j < sortedAnnotations_stem_token.size(); j++) {

						currAnnot = (Annotation) sortedAnnotations_stem_token
								.get(j);
						String annottype = currAnnot.getType();
						if (annottype.equalsIgnoreCase("Token")) {
							mapOffsetToIdStem.put(currAnnot.getStartNode()
									.getOffset(), j);
						}
					}
				}
				...
				AnnotationSet defaultAnnotSet_stemmed = doc_stemmed
						.getAnnotations();
				...
				AnnotationSet annSet_stemmed = defaultAnnotSet_stemmed
						.get(annotTypesRequired_stemmed);
				...
				for (int i = 0; i < sortedAnnotations_stemmed.size(); i++) {
					...
					if (annottype.equalsIgnoreCase("Neg")) {
						Long startOffset = mapIdToOffsetOriginal
								.get(mapOffsetToIdStem.get(Long
										.parseLong(currAnnot.getStartNode()
												.getOffset().toString())));
						...
						int startOff = Integer.parseInt(currAnnot
								.getStartNode().getOffset().toString());
						int endOff = Integer.parseInt(currAnnot.getEndNode()
								.getOffset().toString());
						sentimentWordAttr.setWord(docStr.substring(startOff,
								endOff));
	...
	public static void POSAnalyser(String commentStr, int threadId)
		...
		Set<Annotation> allTokensSet = new HashSet<Annotation>(defaultAnnotSet
				.get(allTokens));
		Set<Annotation> productTokensSet = new HashSet<Annotation>(
				defaultAnnotSet.get(productTokens));
		...
		while (it.hasNext()) {
			...
			Integer j = 1;
			currAnnot = (Annotation) it.next();
			Integer start = Integer.parseInt(currAnnot.getStartNode()
					.getOffset().toString());
			Integer end = Integer.parseInt(currAnnot.getEndNode().getOffset()
					.toString());
			...
			while (it2.hasNext()) {
				...
				Integer startOff = Integer.parseInt(currToken.getStartNode()
						.getOffset().toString());
				Integer endOff = Integer.parseInt(currToken.getEndNode()
						.getOffset().toString());
				String temp = "";
				if (startOff >= start && endOff <= end) {
					...
					sb.append(" ");
					mapOfPOSTags.put(j++, (String) currToken.getFeatures().get(
							"category"));
		...
		while (it.hasNext()) {
			prodAnnot = (Annotation) it.next();
			Integer start = Integer.parseInt(prodAnnot.getStartNode()
					.getOffset().toString());
			Integer end = Integer.parseInt(prodAnnot.getEndNode().getOffset()
					.toString());
	...
	private static SentenceObj getSplits(SentenceObj sentObj) {
		...
		while (resSplit = mSplit.find()) {
			mapOfSplitWords.put(mSplit.start(), sent.substring(mSplit.start(),
					mSplit.end()));
		...
		for (int i = 0; i < splits.length; i++) {
			...
			if (i < splits.length - 1) {
				...
				lastIndex = sent.indexOf(splits[i]) + splits[i].length()
						+ splitWord.length();
			}
		}
	...
	public static void finalAnalyser(int threadId, String chunk)
